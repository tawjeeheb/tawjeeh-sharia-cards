Tawjeeh Sharia Cards Project

## قبل توليد أي بطاقة

```bash
# 1. فحص الروابط أولًا
python3 outputs/check_links.py outputs/test_card_XXX.md

# 2. توليد PDF بعد اجتياز الفحص
python3 outputs/pdf/convert.py
```

## الملفات الحاكمة

| الملف | الغرض |
|-------|-------|
| `outputs/LINK_POLICY.md` | سياسة الروابط المعتمدة لجميع البطاقات |
| `outputs/check_links.py` | سكربت فحص الروابط قبل التوليد |
| `outputs/pdf/convert.py` | قالب التوليد (CSS + شكل الروابط مقفول) |
| `outputs/test_card_001_qadi.md` | البطاقة المرجعية النموذج — Golden Master للتصميم فقط |
| `outputs/link_audit_qadi_v1.md` | ملف تدقيق روابط بطاقة قاضي |
| `outputs/VISUAL_CLEAN_PAGE_LOCK.md` | قفل النظافة البصرية + منطق القوائم الذكي (commit `323493c`) |
| `outputs/READER_FIRST_COURSE_TITLE_LOCK.md` | قفل تسمية الدورات بصيغة قارئية وظيفية عربية |
| `outputs/LINK_VALIDATION_LOCK.md` | قفل دائم لجميع الروابط الخارجية — شروط القبول الإلزامية |
| `outputs/COURSERA_LINK_LOCK.md` | قفل دائم لروابط Coursera — شروط القبول الإلزامية |
| `outputs/ROLE_SPECIFIC_CONTENT_LOCK.md` | قفل حاكم — كل بطاقة تُبنى من الصفر؛ 18 عنصرًا واجب التخصيص؛ اختبار القبول الستة |
| `outputs/CONTENT_DECISION_ENGINE.md` | محرك قرار المحتوى — 9 خطوات إلزامية؛ بطاقة قاضي = Golden Master للتصميم فقط لا للمحتوى |
| `outputs/COURSE_SELECTION_ENGINE.md` | محرك اختيار الدورات — الاختيار يبدأ من المهارة؛ 3 مراحل + اختبار عدم النسخ + اختبار القبول |
| `outputs/CERTIFICATION_SELECTION_ENGINE.md` | محرك اختيار الشهادات — 10 مسارات مهنية؛ يبدأ من المهمة الحرجة لا من الشهرة؛ اختبار القبول |
| `outputs/PROGRAM_SELECTION_ENGINE.md` | محرك اختيار البرامج — يبدأ من فجوة الدخول؛ 10 أنواع فجوة؛ أولوية للبرامج السعودية؛ اختبار القبول |
| `outputs/QUALITY_GATE.md` | بوابة الجودة — آخر خطوة قبل الاعتماد؛ 7 مراحل فحص؛ لا بطاقة تُعتمد بدون اجتيازها |
| `outputs/CONTENT_REGISTRY.md` | سجل مركزي لكل البرامج والشهادات والدورات المستخدمة — يُراجع قبل أي اختيار جديد |
| `outputs/SOURCE_EVIDENCE_LOCK.md` | قفل الأدلة — أي معلومة مهنية بلا دليل قابل للتتبع لا تُذكر |
| `outputs/FINAL_CARD_APPROVAL_CHECKLIST.md` | قائمة الفحص النهائية — 22 بندًا إلزاميًا؛ فشل بند واحد = البطاقة مرفوضة |
| `outputs/CROSS_CARD_CONSISTENCY_ENGINE.md` | محرك الاتساق — 10 معايير أسلوبية تضمن أن جميع البطاقات تبدو من محرر واحد |
| `outputs/SPECIALIZATION_BOUNDARY_LOCK.md` | قفل الحدود التخصصية — لا عنصر يُدرج إلا إذا مورس فعليًا في هذه المهنة داخل السعودية |
| `outputs/EMPLOYMENT_SECTION_PHILOSOPHY.md` | **فلسفة حاكمة** — الهدف مسار الدخول الحقيقي لا أماكن الممارسة؛ عرض تكيفي (نموذج أ/ب)؛ اختبار القيمة الإلزامي؛ منع التضخم |
| `outputs/APPLICATION_CHANNEL_GROUPING_LOCK.md` | **قفل تجميع قنوات التقديم** — الوحدة الأساسية هي طريقة التقديم لا الجهة؛ تجميع الجهات المتشابهة في قناة واحدة؛ منع الافتراض؛ يعمل بعد اختيار الجهات وقبل كتابة العنصر |
| `outputs/APPLICATION_DESCRIPTION_LOCK.md` | **قفل أسلوب طريقة التقديم** — خريطة دخول لا وصف عام؛ 4 عناصر إلزامية متسلسلة؛ افتتاح موحد «التقديم يبدأ عبر...»؛ اختبار جودة 4 أسئلة؛ يعمل آخر خطوة قبل كتابة العنصر النهائي |
